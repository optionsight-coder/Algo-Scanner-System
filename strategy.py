...
def convert_to_3_line_break(df, num_lines=3):
    ...
    # Initialize first line based on first close
    ...
    lines.append({
        ...
        'color': initial_color,
        'date': df.index[0]  # Timestamp tracking for alerts
    })

    # Core TradingView Synthetic Brick Logic
    for i in range(1, len(df)):
        ...
        if last_line['color'] == 'green':
            if current_close > last_line['high']:
                ...
                # Reversal Down (Breaks low of last N lines)
                lines.append({
                    'open': last_line['close'],
                    'close': current_close,
            ...
            if current_close < last_line['low']:
                ...
                # Reversal Up (Breaks high of last N lines)
                lines.append({
                    'open': last_line['close'],
                    'close': current_close,
                    'high': current_close,
    ...
    # Convert synthetic bricks back to a DataFrame for EMA calculations
    lb_df = pd.DataFrame(lines)
    lb_df.set_index('date', inplace=True)
    # Rename 'close' to 'Close' so pandas_ta can read it seamlessly
